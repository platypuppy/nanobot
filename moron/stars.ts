 ...
 */
import { Client, EmbedBuilder, Emoji, GuildEmoji, Message, MessageType, PartialMessage, ReactionEmoji, TextChannel } from 'discord.js';
const { iconicMemes, serverLog } = require('../groche-channels.json');
import * as fs from 'fs';
import { pinnedMessagesLoaded } from '..';
import { Logger, WarningLevel } from './logger';
const { StarDBFolder, StarDBFile, ReactsToTrigger } = require('../config/stars.json');
...
async function pinMessage(client : Client<boolean>, reactType: GuildEmoji | ReactionEmoji, post: Message<boolean> | PartialMessage, count: number)
{
	...
	let postEmbed = new EmbedBuilder()
		...
		.setAuthor({
			...
			iconURL: post.author?.avatarURL() ?? 'https://cdn.discordapp.com/embed/avatars/5.png',
		})
		.setDescription(msgContent)
		.setFooter({text: (reactType.id ? '' : reactType.name + ' ') + getReactText(), iconURL: reactType.url ?? undefined})
	...
	if (post.attachments.size > 0)
	{
		...
		for (let i = 1; i < post.attachments.size; i++)
		{
			...
			{ // sending multiple embeds with the same URL nests them in the same post
				additionalAttachments.push(new EmbedBuilder().setURL(post.url).setImage(nUrl));
			}
		}
	}
	
	const starChannel = client.channels.cache.get(devMode ? serverLog : iconicMemes);
	(starChannel as TextChannel).send({embeds: additionalAttachments});
}

export function stars_onStarAdded(client: Client<boolean>, reactType: GuildEmoji | ReactionEmoji, post: Message<boolean> | PartialMessage, count: number | null) {
	if (count !== null)
	{
...
{
	return (devMode || Math.random() > 0.5) ? reactTexts[Math.floor(Math.random() * reactTexts.length)] : ' ';
}
