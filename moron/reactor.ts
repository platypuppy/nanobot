...
const { emoteHmmm, emoteJii, emoteSethCP, emoteMajimeHi } = require('../groche-emotes.json');
...
export async function reactor_onMessageSend(msg: Message)
{
	if (!client.user)
	{
		logger.log('Tried to receive message when client was not initialized!', WarningLevel.Error);
		return;	
	}
